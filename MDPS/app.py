...
BASE_DIR = Path(__file__).resolve().parent  # → …/MDPS

def load_model(name):
    """Return (model, scaler) tuple stored in project_model/<name>.pkl"""
    model_path = BASE_DIR / "project_model" / f"{name}.pkl"
    with model_path.open("rb") as f:
        return pickle.load(f)

diabetes_model, scaler1   = load_model("diabetes_model")
parkinsons_model, scaler2 = load_model("parkinson_model")
heart_model, scaler3      = load_model("heart_disease_model")
...
if (selected == 'Heart Disease Prediction'):
    ...
    if st.button('Heart Disease Test Result'):
        ...
        heart_disease_model_prediction = heart_model.predict(std_data_2)
...
if (selected == 'Parkinsons Prediction'):
    ...
    if st.button('Parkinsons Disease Test Result'):
        ...
        parkinson_model_prediction = parkinsons_model.predict(std_data_3)
